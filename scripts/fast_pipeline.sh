...
MAX_IMG=1000     # Max image dimension for dense reconstruction
FEAT_IMG=500     # Max image dimension for feature extraction (smaller = faster SfM)
...
S=$(date +%s%N)
ffmpeg -i "$INPUT_VIDEO" -vf "fps=$FPS,scale=$MAX_IMG:-2" -q:v 4 "$WS/images/frame_%05d.jpg" -loglevel warning -y 2>&1
...
$COLMAP feature_extractor \
  ...
  --FeatureExtraction.max_image_size $FEAT_IMG \
